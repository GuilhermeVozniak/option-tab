...
Current native limitation: even `SMOKE_RETAIN_CLOSED=0` logs removal from the fixture owner array without a `SmokeWindow.dealloc` notification; macOS still retains the closed CG surface. Both modes therefore intentionally fail the stale-window inventory assertion on the tested OS. Native close/AX omission and positive stream cleanup are logged independently. This mode is not presented as proof of actual NSWindow destruction.
