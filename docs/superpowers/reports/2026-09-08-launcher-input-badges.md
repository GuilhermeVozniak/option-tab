...
Two test-only tools prepare the coordinated native session. The controlled badge fixture changes only its own badge, while an explicitly opted-in test observes its exact process and bundle through the production source. The standalone Wails panel measures manually supplied gesture and committed-text delivery through production native ports. It uses an interior surface and performs no launcher actions. Both tools compiled without being launched; the badge test's default path was verified to skip immediately. Their READMEs document explicit invocation, bounded lifetime, cleanup and limits. These preparation checks add no live acceptance evidence and change no production capability gates.
