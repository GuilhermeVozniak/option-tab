...
Keep a process-local monotonic `settingsRevision` owned by `saveMu`, initialized nonzero after settings load and incremented only after each successful `saveSettingsLocked` publication. Add revision-aware Preferences transport while retaining legacy methods only for compatibility:
...
`GetSettingsState` snapshots revision and settings coherently under `saveMu`; `SaveSettingsAtRevision` decodes strictly, locks `saveMu`, compares the expected revision, saves once, advances the revision and returns the canonical committed snapshot. Runtime `MutateLauncherItems` captures the current settings revision only after acquiring `saveMu`, mutates that fresh snapshot, then saves under the same lock. It therefore cannot overwrite a concurrent Preferences save. The revision need not be persisted across process restarts because no renderer survives that boundary.

`OpenPreferences` emits a copied revisioned settings snapshot after marking Preferences open, even when reusing an existing webview. `useSettingsModel` admits this snapshot by monotonic revision and replaces its hidden stale model before enabling edits; a pending save from an older revision is refused by App CAS and reloads canonical settings instead of retrying the stale full JSON. Successful runtime mutations may additionally emit the same revisioned snapshot, but no background frontend polling or second persistence queue is needed. Closing/reopening Preferences and a runtime mutation must never leave an old controlled form enabled against a newer App revision.

Groups retain current config invariants: only app items, one group per member, no empty group, maximum 16 top-level configured records. Moving a group moves it atomically; moving a member within its group changes member order. `itemsRevision` is a content hash and may return to A after A→B→A. Every accepted mutation must still advance the launcher owner/presentation revision (or create a successor session), and action/drag admission checks that exact scope as well as the hash. Thus a gesture admitted against the first A cannot act on the later A even though their content hashes match.
...
Letter navigation is opt-in and scoped to the visible panel. Add a native panel key-forwarding packet `{scope,sequence,key,modifiers}` only if nonactivating-panel key delivery is proven. Accept unmodified printable Unicode letters, case-fold and cycle prefix matches among current visible actionable items; repeated same letter advances, 750 ms idle resets. Ignore composing input, modifiers, decorations, unavailable items and hidden/tombstoned sessions. It changes local selection only; Enter activation remains a separate explicit key action and defaults disabled. No global event tap or shortcut registration.
